...
MainWindow::MainWindow() {
    ...
    uname   = QString::fromStdString(value["uname"].get<std::string>());
    pfp_b64 = QString::fromStdString(value["pfp"].get<std::string>());
    passwd  = QString::fromStdString(value["passwd"].get<std::string>());
    ...
    addServerButton->setIcon(QIcon("add.png"));
    ...
    connect(settings, &SettingsMenu::unameChanged, this, [this](QString newVal) { this->uname = newVal; this->updateMeta(); });
    connect(settings, &SettingsMenu::passwdChanged, this, [this](QString newVal) { this->passwd = newVal; this->updateMeta(); });
    connect(settings, &SettingsMenu::pfpChanged, this, [this](QString newVal) { this->pfp_b64 = newVal; this->updateMeta(); });
    setLayout(layout);
    input->setFocus();
    show();
}
